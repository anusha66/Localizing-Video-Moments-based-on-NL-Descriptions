...
rn_path = 'i3d_global_features'
rn_path2 = 'i3d_global_predictions'
...
def evaluate_sample():
 ...
 #with h5py.File("fc_i3d_max_fea_xaa.hdf5", "w") as f_fea:
  ...
  data_file = 'global_new'
  fff = open(data_file, 'r')
  for vname in fff:
  #for vname in glob.glob('../VIDEO_DIRECTORY/*'):
    vid_name = '34064267@N00_9221249101_0e0dc55581.mov'
    #13549533@N02_3355384254_1eef134b0c.mov'
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
     ...
     np.savez(rn_filename, all_fea)
     np.savez(rn_filename2, all_pred)
     #pdb.set_trace()
     #print(out_logits[0])
...
def main():
    ...
    with open(data_file, 'r') as file:
        ...
        #for vname in glob.glob('../VIDEO_DIRECTORY/*'):
            ...
            tf.app.run(main=evaluate_sample, argv=[feat])
            print(all_fea) 
