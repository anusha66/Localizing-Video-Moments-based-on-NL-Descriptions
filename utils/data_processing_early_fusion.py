...
class extractVisualFeatures(extractData):
  ...
  def __init__(self, dataset, params, result):
    ...
    features_h5py_flow = h5py.File(params['features_flow'])
    features_h5py_rgb = h5py.File(params['features_rgb'])
    features = {}
    for key in features_h5py_rgb.keys():
      rgb = np.array(features_h5py_rgb[key])
      flow  = np.array(features_h5py_flow[key])
      features[key] = np.hstack((rgb,flow))
    features_h5py_rgb.close()
    features_h5py_flow.close()
